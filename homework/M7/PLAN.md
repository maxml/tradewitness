...
- **Приёмка:** после демо в дашборде видно все 6–10 прогонов с колонками; PII замаскирован по умолчанию **и в message, и в response** (в т.ч. PII из БД в ответе, которого не было в запросе); **fail-closed маскирование**: при `redactionStatus !== "ok"` сервер вообще не отдаёт текст (поле `null`), UI рисует `<REDACTION_UNAVAILABLE>` (сырой/regex-текст в client-payload не попадает); при выключенном Reveal **client-payload не содержит raw-полей** (только `redacted*`); **авторизация = 403 на уровне данных** (`getChatLogsForAdmin` бросает `forbidden`; страница рендерит Access-Denied UI как и остальные admin-страницы — RSC не умеет ставить 403 на успешный рендер); `prune` реально удаляет старые записи (проверить на записи с подделанным `createdAt`).
